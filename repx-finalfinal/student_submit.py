...
import os
import fitz
import pyspellchecker.spellchecker
import requests
...
#program run
def get_answer(code,scan_ans,total):
    ...
    total_text_marks=1*total

    if isdiagram:
        total_diagram=total*0.2
        total_text_marks=0.8*total
        ...
        if round(similarity, 2)>0.95:
            diagram_marks=1*total_diagram
        elif round(similarity, 2)>0.9:
            diagram_marks=0.8*total_diagram
        elif round(similarity, 2)>0.85:
            diagram_marks=0.6*total_diagram
        elif round(similarity, 2)>0.8:
            diagram_marks=0.4*total_diagram
        elif round(similarity, 2)>0.75:
            diagram_marks=0.2*total_diagram
        else:
            diagram_marks=0
    ...
    marks=0
    if sim>0.99:
        marks=1*total_text_marks
    elif sim>0.98:
        marks=0.9*total_text_marks
    elif sim>0.97:
        marks=0.8*total_text_marks
    elif sim>0.95:
        marks=0.7*total_text_marks
    elif sim>0.92:
        marks=0.6*total_text_marks
    elif sim>0.90:
        marks=0.5*total_text_marks
    elif sim>0.87:
        marks=0.4*total_text_marks
    elif sim>0.82:
        marks=0.3*total_text_marks
    elif sim>0.78:
        marks=0.2*total_text_marks
    else:
        marks=0

    obtained_marks=diagram_marks+marks
  
    # print("Total Marks out of ",total,":",obtained_marks)
